...
#
# GovernanceHub
# - Controller/registry for Proposal and Comment templates (cloned via minimal proxies)
# - 5-admin model: 3 elected, 1 creator, 1 bobu multisig (god mode)
# - O(1) pagination for proposals by state using offset/count + reverse
# - Time-based and admin-forced state transitions
# - Optional ERC-1155 gating for propose/comment/vote (toggleable)
#
# Notes:
# - Returning k items from pagination costs O(k) gas due to return copy; boundaries are O(1).
# - Bobu multisig has authority to reset all admins and modify critical config.

interface IERC1155:
    def balanceOf(owner: address, id: uint256) -> uint256: view

interface IProposalTemplate:
    def initialize(_hub: address, _title: String[128], _author: address, _body: String[1024], _createdAt: uint256, _voteStart: uint256, _voteEnd: uint256): nonpayable
...
interface ICommentTemplate:
    def initialize(_hub: address, _proposal: address, _author: address, _content: String[1024], _createdAt: uint256): nonpayable
...
COMMENT_DELETE_WINDOW: constant(uint256) = 14 * 86400  # 2 weeks

# Admins
bobuMultisig: public(address)  # god mode
...
# O(1) indexing helpers (offset-by-one trick)
stateByProposalPlusOne: HashMap[address, uint256]   # state + 1
indexByProposalPlusOne: HashMap[address, uint256]   # index + 1
...
def _onlyAdminOrBobu():
    assert self.isAdmin(msg.sender), "admin required"
...
# Bobu can rotate itself (must be non-zero to avoid bricking)
@external
def setBobuMultisig(_newBobu: address):
    self._onlyBobu()
    assert _newBobu != empty(address), "zero bobu"
    old: address = self.bobuMultisig
    self.bobuMultisig = _newBobu
    log BobuChanged(old, _newBobu)

# Bobu updates elected admins
@external
...
# Token requirement (gate toggles are separate)
@external
...
def _requireProposer(user: address):
    if self.gateProposals:
        assert self.hasToken(user), "token required to propose"

@internal
@view
def _requireCommenter(user: address):
    if self.gateComments:
        assert self.hasToken(user), "token required to comment"

@internal
@view
def _requireVoter(user: address):
    if self.gateVotes:
        assert self.hasToken(user), "token required to vote"
...
def _appendToState(p: address, st: uint256):
    ...
    if st == STATE_DRAFT:
        ...
        self.indexByProposalPlusOne[p] = len(self.draftProposals)  # idx+1
    elif st == STATE_OPEN:
...
def _removeFromState(p: address):
    ...
    if st == STATE_DRAFT:
        last_idx_d: uint256 = len(self.draftProposals) - 1
        if idx != last_idx_d:
            last_addr_d: address = self.draftProposals[last_idx_d]
            self.draftProposals[idx] = last_addr_d
            self.indexByProposalPlusOne[last_addr_d] = idx + 1
        self.draftProposals.pop()
    elif st == STATE_OPEN:
        last_idx_o: uint256 = len(self.openProposals) - 1
        if idx != last_idx_o:
            last_addr_o: address = self.openProposals[last_idx_o]
            self.openProposals[idx] = last_addr_o
            self.indexByProposalPlusOne[last_addr_o] = idx + 1
        self.openProposals.pop()
    elif st == STATE_ACTIVE:
        last_idx_a: uint256 = len(self.activeProposals) - 1
        if idx != last_idx_a:
            last_addr_a: address = self.activeProposals[last_idx_a]
            self.activeProposals[idx] = last_addr_a
            self.indexByProposalPlusOne[last_addr_a] = idx + 1
        self.activeProposals.pop()
    else:
        last_idx_c: uint256 = len(self.closedProposals) - 1
        if idx != last_idx_c:
            last_addr_c: address = self.closedProposals[last_idx_c]
            self.closedProposals[idx] = last_addr_c
            self.indexByProposalPlusOne[last_addr_c] = idx + 1
...
@external
def createProposal(_title: String[128], _body: String[1024], _voteStart: uint256, _voteEnd: uint256) -> address:
    self._requireProposer(msg.sender)
    if _voteEnd > 0:
        assert _voteStart > 0, "end with no start"
        assert _voteEnd > _voteStart, "invalid window"

    p: address = create_minimal_proxy_to(self.proposalTemplate, revert_on_failure=True)
    IProposalTemplate(p).initialize(self, _title, msg.sender, _body, block.timestamp, _voteStart, _voteEnd)

    # initial bucket by time (support indefinite window if end==0 and start>0)
    target_state: uint256 = STATE_DRAFT
    if _voteStart > 0 and _voteEnd == 0:
        if block.timestamp < _voteStart:
            target_state = STATE_OPEN
        else:
            target_state = STATE_ACTIVE
    elif _voteStart > 0 and _voteEnd > 0:
        if block.timestamp < _voteStart:
            target_state = STATE_OPEN
...
def addComment(_proposal: address, _content: String[1024]) -> address:
    ...
    ICommentTemplate(c).initialize(self, _proposal, msg.sender, _content, block.timestamp)
    IProposalTemplate(_proposal).addCommentAddress(c)
...
def castVote(_proposal: address, support: bool):
    ...
    assert self.stateByProposalPlusOne[_proposal] > 0, "unknown proposal"
    vs: uint256 = IProposalTemplate(_proposal).voteStart()
    ve: uint256 = IProposalTemplate(_proposal).voteEnd()
    assert vs > 0, "no voting start"
    assert block.timestamp >= vs, "not started"
    if ve > 0:
        assert block.timestamp <= ve, "ended"
    # 1 address = weight 1 (can be extended later)
    IProposalTemplate(_proposal).hubCastVote(msg.sender, support, 1)
...
def syncProposalState(_proposal: address):
    ...
    if vs == 0 and ve == 0:
        ...
        if block.timestamp < vs:
            new_st = STATE_OPEN
        else:
            new_st = STATE_ACTIVE
    elif vs > 0 and ve > 0:
        if block.timestamp > ve:
            new_st = STATE_CLOSED
        elif block.timestamp >= vs:
            new_st = STATE_ACTIVE
        else:
            new_st = STATE_OPEN
    else:
        # malformed window (ve > 0 and vs == 0), treat as DRAFT
        new_st = STATE_DRAFT
...
def adminDeleteComment(_proposal: address, _comment: address):
    self._onlyAdminOrBobu()
    assert self.stateByProposalPlusOne[_proposal] > 0, "unknown proposal"
    assert ICommentTemplate(_comment).proposal() == _proposal, "not linked"
    created: uint256 = ICommentTemplate(_comment).createdAt()
    assert block.timestamp <= created + COMMENT_DELETE_WINDOW, "window passed"
    ICommentTemplate(_comment).markDeleted()
...
def getProposals(_state: uint256, _offset: uint256, _count: uint256, reverse: bool) -> DynArray[address, PAGE_LIMIT]:
    ...
    arr_len: uint256 = self.getProposalCountByState(_state)
    if arr_len == 0:
        return result
    ...
    if not reverse:
        ...
        available_rev: uint256 = start_index + 1
        count_rev: uint256 = min(min(_count, available_rev), PAGE_LIMIT)
        for i: uint256 in range(0, count_rev, bound=PAGE_LIMIT):
